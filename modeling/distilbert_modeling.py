...
from classifiers.distilbert_model import MyDistilbertClassifier
...
if __name__ == "__main__":
    ...
    # modeling
    distilbert_classifier = MyDistilbertClassifier(model_name, params_dict, MAX_SEQ_LEN, num_labels=2)
    distilbert_classifier.build_classifier()
    distilbert_classifier.compile_classifier()
    distilbert_classifier.model.summary()

    # preprocess data for distilbert
    tokenized_train = distilbert_classifier.preprocess_data(x_train)
    tokenized_valid = distilbert_classifier.preprocess_data(x_valid)
    tokenized_test = distilbert_classifier.preprocess_data(x_test)

    start_time = time.time()
    logger.info(f'Training the model...')
    distilbert_classifier.fit_model(tokenized_train, y_train, tokenized_valid, y_valid)
    ...
    # reloaded_distilbert_classifier = load_model((os.path.join(os.getcwd(), 'models', model_name + '.h5')), custom_objects={'TFDistilBertForSequenceClassification': TFDistilBertForSequenceClassification})
    
    # validation
    start_time = time.time()
    logger.info(f'Validating the model...')
    validate_classifier(distilbert_classifier, dict(tokenized_test), y_test)
    ...
    x_essays = distilbert_classifier.preprocess_data(x_essays.values.tolist())
    validate_classifier(distilbert_classifier, dict(x_essays), y_essays, new_data=True)
    ...
    x_extras = distilbert_classifier.preprocess_data(x_extras.values.tolist())
    validate_classifier(distilbert_classifier, dict(x_extras), y_extras, new_data=True)
